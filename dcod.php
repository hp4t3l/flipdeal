...
if(isset($_SESSION['user']))
{
	...
	{
		$noofitem1=mysqli_query($con,"select count(*) from cart where userid='$uid' and status='yes'");
		...
		$f1=mysqli_query($con,"select cart.ptdid,quantity,product.price,cartid from cart left join product on cart.ptdid=product.ptdid where cart.userid='$uid' and cart.`status`='yes';");
		$cf1=mysqli_query($con,"select count(*) from cart left join product on cart.ptdid=product.ptdid where cart.userid='$uid'and `cart`.`status`='yes';");
...
mysqli_query($con,"update product set stock=$st where ptdid=$f[0]") or die("update stock");
	...
	echo "<div class='guli'><ul><li><table align='center' border='0' cellspacing='30'><tr><th colspan='5'><h1 align='center'>INVOICE</h1></th></tr><tr><th>ORDER ID</th><TD aling='center' colspan='4'>".$order[0]."</td></tr>";
	...
	mysqli_query($con,"delete from cart where userid='$uid'and `status`='yes'");
?>
